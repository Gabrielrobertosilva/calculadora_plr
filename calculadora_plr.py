# calculadora_plr.py – v3 (somente regra de antecipação PLR)
...
data_assinatura_cct = st.sidebar.date_input(
    ...
    help="Usada nas regras de elegibilidade dos parágrafos e do caput."
)
...
lucro_liquido_1s2025 = st.sidebar.number_input(
    ...
    help="Base do teto global (12,8%) e da parcela adicional (2,2%). Se for 0, os limites zeram o pagamento."
)
compensar_planos_proprios = st.sidebar.checkbox(
    "Compensar valores já pagos em 2025 (Regra Básica)", value=False
)

# (opcionais para simulações livres)
...
def calcular_proporcionalidade_especial(row, data_assinatura):
    ...
    # §1º – admitido até 31/12/2023, afastado, no quadro na assinatura → integral
    ...
    # §2º – admitido a partir de 01/01/2024, em efetivo exercício na assinatura (mesmo afastado) → proporcional por 1/12 até 31/12/2024
    ...
    if not calcular:
        ...
        if base.empty:
            ...
            base_calc["Proporcionalidade"] = base_calc.apply(
                lambda r: calcular_proporcionalidade_especial(r, data_assinatura_cct), axis=1
            )
            ...
            if n_elegiveis == 0:
                ...
                if pool_adic == 0:
                    st.warning("O lucro 1S/2025 está 0. A Parcela Adicional (2,2%) também será 0.")
